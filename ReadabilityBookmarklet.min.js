...
for(var m=0;m<this._nbTopCandidates;m++){var _=h[m]
if(!_||p>_.readability.contentScore){h.splice(m,0,u),h.length>this._nbTopCandidates&&h.pop()
break}}}var f,N=h[0]||null,y=!1
...
if(v.length>=3)for(f=N.parentNode;"BODY"!==f.tagName;){for(var T=0,A=0;A<v.length&&T<3;A++)T+=Number(v[A].includes(f))
if(T>=3){N=f
break}f=f.parentNode}N.readability||this._initializeNode(N),f=N.parentNode
for(var x=N.readability.contentScore,S=x/3;"BODY"!==f.tagName;)if(f.readability){var C=f.readability.contentScore
if(C<S)break
if(C>x){N=f
break}x=f.readability.contentScore,f=f.parentNode}else f=f.parentNode
for(f=N.parentNode;"BODY"!=f.tagName&&1==f.children.length;)N=f,f=N.parentNode
...
f=N.parentNode
for(var I=f.children,D=0,B=I.length;D<B;D++){var w=I[D],R=!1
...
L.appendChild(H)}if(this._debug&&this.log("Article content after paging: "+L.innerHTML),!(this._getInnerText(L,!0).length<500)){var F=[f,N].concat(this._getNodeAncestors(f))
...
style.type="text/css",style.appendChild(document.createTextNode("div#pages{white-space: pre-wrap;} #pages .turn-page{ background-color:#ccc; background-size:100% 100%; }")),document.getElementsByTagName("head")[0].appendChild(style)
var readyFlag=!1,initTurnJS=function(e){var t=e(window).height(),i=e(window).width()
e("#pages").turn({display:"single",acceleration:!0,gradients:!0,duration:0,height:t,width:i,when:{turned:function(e,t){}}}),readyFlag=!0}
window.injectScript("https://code.jquery.com/jquery-2.1.0.min.js"),loadScript("https://cdn.rawgit.com/blisszard/turn.js/08c1f659/turn.min.js",function(){jQuery(function(e){e(window).ready(function(){initTurnJS(e)}),e("#pages").on("touchstart click",function(t){t.stopPropagation(),t.preventDefault()
var i=(e(window).height(),e(window).width())
t.pageX<=i/2?e("#pages").turn("previous"):e("#pages").turn("next")}),e(window).bind("keydown",function(t){37==t.keyCode?e("#pages").turn("previous"):39==t.keyCode&&e("#pages").turn("next")})})}),document.body.innerHTML='<div id="pages">'+jQuery(content).text()+"</div>"
